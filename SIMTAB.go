...
func ubahTagihan(A *tabDataTagihan, n int) {
	...
	if n == 0 {
		...
		if posisi == -1 {
			...
			fmt.Printf("[1] Nama            : %s", A[posisi].nama)
			fmt.Printf("[2] Nomina          : Rp%2.f", A[posisi].nominal)
			tgl := A[posisi].jatuhTempo % 100
			bln := A[posisi].jatuhTempo / 100 % 100
			thn := A[posisi].jatuhTempo / 10000
			fmt.Printf("[3] Jatuh Tempo     : %2d/%2d/%4d", tgl, bln, thn)
			fmt.Printf("[4] Kategori        : %s", A[posisi].nama)
			fmt.Printf("[5] Status          : %s", A[posisi].nama)
	...
	if n == 0 {
		...
		if cari == "Nama" || cari == "nama" {
			...
			if !found {
				...
				fmt.Printf("Dietmukan sebanyak %d tagihan dengan nama [%s]\n", no-1, cariNama)
			}
			fmt.Println()
			...
			if !found {
				fmt.Println("Tagihan dengan kategori [%s] tidak ditemukan\n!", cariKategori)
			} else {
				fmt.Println("Dietmukan sebanyak %d tagihan dengan kategori [%s]\n", no-1, cariKategori)
			}
			fmt.Println()
		}
	}
...
func lihatStatistik(A tabDataTagihan, n int) {
	...
	if n == 0 {
		...
		fmt.Printf("Total Seluruh Biaya          : %2.f tagihan\n", totBiaya)
		fmt.Printf("Total Biaya Lunas            : %2.f tagihan\n", totLunas)
		fmt.Printf("Total Biaya Belum Lunas      : %2.f tagihan\n", totBelumLunas)

		fmt.Println("═══ Informasi Persentase ═══")
		fmt.Printf("Persentase Lunas            : %2.f tagihan\n", persenLunas)
		fmt.Printf("Persentase Belum Lunas      : %2.f tagihan\n", persenBelumLunas)
